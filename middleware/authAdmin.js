...
const authAdmin = (req,res,next) => {
    try {
        ...
        console.log(token)
        if (!token) {
        return res.status(400).json({ message: "Ошибка: нет ключа авторизации" })
        }
        ...
        return res.status(400).json({ message: "У Вас нет доступа для выполнения этого действия" })
        }
    } catch (error) {
        console.log('auth error:', error)
        return res.status(400).json({ message: "Проблема с модулем authAdmin.js" })
      }
      next();
}
